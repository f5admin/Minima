...
install() {
	if [ "$port" = "9001" ]; then
		...
		. <(wget -qO- https://raw.githubusercontent.com/f5eleven/main/insert_variable.sh) -n "minima_node_info" -v ". <(wget -qO- https://raw.githubusercontent.com/f5eleven/Minima/main/node_info.sh) -l EN 2> /dev/null" -a
	else
		local node_name="minima_${port}"
		. <(wget -qO- https://raw.githubusercontent.com/f5eleven/main/insert_variable.sh) -n "${node_name}_node_info" -v ". <(wget -qO- https://raw.githubusercontent.com/f5eleven/Minima/main/node_info.sh) -l EN -p $((port+1)) 2> /dev/null" -a
	fi
	...
	. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/main/ports_opening.sh) "$port" $((port+1))
	printf "[Unit]
...
WantedBy=multi-user.target" > "/etc/systemd/system/${node_name}.service"
	...
	. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/main/insert_variable.sh) -n "${node_name}_log" -v "sudo journalctl -fn 100 -u ${node_name}" -a
...
docker_install() {
	if [ "$port" = "9001" ]; then
		local node_name="minima"
		. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/main/insert_variable.sh) -n "minima_node_info" -v ". <(wget -qO- https://raw.githubusercontent.com/f5eleven/Minima/main/node_info.sh) -l EN 2> /dev/null" -a
	else
		local node_name="minima_${port}"
		. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/main/insert_variable.sh) -n "${node_name}_node_info" -v ". <(wget -qO- https://raw.githubusercontent.com/f5eleven/Minima/main/node_info.sh) -l EN -p $((port+1)) 2> /dev/null" -a
	fi
	...
	if [ -n "$is_docker" ]; then
		...
		printf_n "${C_LGn}Node installation...${RES}"
		. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/main/docker.sh)
		docker run -dit --restart on-failure --name "${node_name}_node" -p $port:9001 -p $((port+1)):9002 secord/minima
	fi
	. <(wget -qO- https://raw.githubusercontent.com/f5eleven/logo/main/insert_variable.sh) -n "${node_name}_log" -v "docker logs ${node_name}_node -fn 100" -a
	printf_n "${C_LGn}Done!${RES}\n"
	. <(wget -qO- https://raw.githubusercontent.com/f5leven/logo/main/logo.sh)
	printf_n "
The node was ${C_LGn}started${RES}.
